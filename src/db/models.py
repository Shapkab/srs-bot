...
class Card(Base):
    ...
    # IPA pronunciation of `front`, generated by OpenAI at card creation
    # (regenerated on /edit when front changes). NULL only for cards
    # predating revision 0007 that the backfill script hasn't reached.
    # Declared last to match the column order ALTER TABLE produces.
    front_pronunciation: Mapped[str | None] = mapped_column(Text, nullable=True)
